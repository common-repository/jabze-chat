    ...
    if (typeof define === 'function' && define.amd) {
        require.config({
            paths: {
                ...
                "strophe.ping": "Libraries/strophe.ping"
            ...
            shim: {
                ...
                'strophe.ping':     { deps: ['strophe', 'jquery'] }
            }
        });
   ...
                var numericKeys = [1, 4, 5, 6, 7, 10, 11],
        ...
        this.ChatBoxView = Backbone.View.extend({
            ...
            events: {
                ...
                'scroll .chat-content':'showClearHistory',
                'click .clear-chatbox-history':'clearHistory'
            },

            message_template: _.template(
                                '<div class="chat-message {{extra_classes}}">' +
                                    '<span class="chat-message-{{sender}}">{{formatTime}} {{username}}:&nbsp;</span>' +
            ...
            appendMessage: function ($el, msg_dict) {
                ...
                $el.append(
                    template({
                        'sender': sender,
                        'time': this_date,
                        'formatTime':this.formatDate(this_date,'H:i P'),
                        'message': text,
                        'username': username,
            ...
            clearHistory:function(){
                if(confirm('Are you sure you want to delete that history')){
                    this.$el.find('.clear-chatbox-history').hide();
                    var length = this.model.messages.length;
                    for (var i = length - 1; i >= 0; i--) {
                        this.model.messages.at(i).destroy();
                    }
                    var $chat_content = this.$el.find('.chat-content');
                    $chat_content.empty();
                }
            ...
            sendMessage: function (text) {
	            ...
	            if (window.mixpanel) {
		            mixpanel.track("wp: Send message.");
	            }
            },
            ...
            initialize: function (){
                ...
                var $clear=this.$el.find('.clear-chatbox-history');
                var ths=this;
                var $content=this.$el.find('.chat-content');
                var timer=null;
                $content.scroll(function() {
                    if($(this).html().length){
                        timer=ths.showClearHistory();
                    }
                });
            ...
            showClearHistory:function(){
                var $clear=this.$el.find('.clear-chatbox-history');
                if (!$clear.data('fadeIn')) {
                    $clear.data('fadeIn', 1).fadeIn();
                }
                return this.throttle('show',function(){$clear.data('fadeIn', 0).fadeOut();},1000);
            },
            throttle :function(name, fn, delay){
                this.timer = this.timer || {};
                this.timer[name] = this.timer[name] || null;
                delay = typeof delay == 'undefined' ? 500 : delay;

                clearTimeout(this.timer[name]);

               return this.timer[name] = setTimeout(function(){
                    fn.call(null);
                },delay);
            },
            template: _.template(
                ...
                '</div><div class="clear-chatbox-history"><span>Clear All</span></div>' +
        ...
        this.ContactsPanel = Backbone.View.extend({
            ...
            tab_template: _.template('<li><a class="s current" href="#users">'+__('My Contacts')+'</a></li>'),
            template: _.template(
                '<form class="set-xmpp-status" action="" method="post" style="display:none">'+
                    ...
                    '</span>'+
                '</form>'+
                '<dl class="add-converse-contact dropdown" style="display:none">' +
        ...
        this.RosterItems = Backbone.Collection.extend({
            ...
            getNumOnlineContacts: function () {
                ...
                for (i=0; i<models_length; i++) {
                    if (_.indexOf(['offline', 'unavailable','xa'], models[i].get('chat_status')) === -1) {
                        count++;
                    }
                }
                return count;
            ...
            presenceHandler: function (presence) {
                ...
                var jid = $presence.attr('from'),
                    ...
                    item;
                var chat_status = $show.text() || 'online';
                ...
                if ((presence_type === 'subscribed') || (presence_type === 'unsubscribe')) {
                    ...
                    if("online"==chat_status){
                        $("#online-count").toggleClass("rotate");
                    }
        ...
        this.LoginPanel = Backbone.View.extend({
            ...
            connect: function ($form, jid, password) {
                ...
                if ($form) {
                    ...
                    $button.hide().after('<img class="spinner login-submit" src="../css/images/spinner.gif"/>');
                }
        ...
        this.giveFeedback = function (message, klass) {
            ...
            if(__('Disconnecting')===message){
                $('#toggle-controlbox').removeClass('controlbox-online');
                converse.hideControlBox();
            }
